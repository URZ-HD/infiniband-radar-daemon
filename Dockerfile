...
# using local .deb because online source were too unstable
COPY libcurl4_7.74.0-1.3+deb11u10_amd64.deb .
COPY libcurl4-openssl-dev_7.74.0-1.3+deb11u10_amd64.deb .

RUN apt update && \
    apt install -y cmake make g++ libibverbs-dev libibnetdisc-dev libibmad-dev libopensm-dev \
    /libcurl4-openssl-dev_7.74.0-1.3+deb11u10_amd64.deb /libcurl4_7.74.0-1.3+deb11u10_amd64.deb && \
...
COPY --from=builder /libcurl4_7.74.0-1.3+deb11u10_amd64.deb /

RUN apt update && \
    apt install -y libibnetdisc5 \
    /libcurl4_7.74.0-1.3+deb11u10_amd64.deb && \
    rm -f /libcurl4_7.74.0-1.3+deb11u10_amd64.deb && \
    rm -rf /var/lib/apt/lists/*
